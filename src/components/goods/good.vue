<template>
  <div class="goods">
    <div class="goods__list" ref="goodsList">
      <div class="goods__list-content">
        ...
        <div class="goods_menu">
          <ul>
            <li class="goods__list-li" v-for="(item,index) in goods" @click="openDetail(item,$event)" :key="index">
              ...
              <div class="goods__list-detail">
                ...
                <p class="goods__list-ptag">{{item.sub_title}}</p>
              </div>
            </li>
          </ul>
        </div>
    ...
    <!-- 详情 -->
    <goods-detail ref="detail" :seller="seller" :selectGoods="selectGoods"></goods-detail>
  </div>
</template>
    ...
    getHomeList(){
      ...
      getHomeList().then(res => {
        ...
        if (code !== 200) {
          ...
          sessionStorage.setItem('homeList', JSON.stringify(res.data.list))
          this.$store.commit('setHomeList', res.data.list)
          this.listLoading = false
        }
      })
    },
