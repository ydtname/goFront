  ...
  <transition name="fadeleft">
    <div class="myfourite">
      <div class="goods__list">
        <ul>
          <li class="goods__list-li" v-for="(item, index) in getCollect" :key="index">
      ...
      <div class="goods__list" v-show="show">
        <h2 class="list__title">╭(╯ε╰)╮去精选里面添加内容吧</h2>
      </div>
    </div>
  </transition>
</template>
...
export default {
  ...
  computed: {
    // 得到收藏的数据
    getCollect() {
      let collectList = this.$store.state.homeList.filter(element => {
        return this.selectnum.some(item => item === element._id)
      });
      return collectList
