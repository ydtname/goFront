      ...
      <div class="left" ref="left">
        <ul class="left__menu">
          <li
            ...
            class="left__list border-1px"
            :key="index"
            :class="{'active':currentIndex===index}"
            @click="selectMenu(index,$event)"
          >
            <p class="left__city" :class="{'textcolor':currentIndex===index}">{{item.country}}</p>
            <p class="left__citynum" :class="{'textcolor':currentIndex===index}">{{item.cityNum}}个目的地</p>
          </li>
        </ul>
      </div>
      <!-- 右边 -->
      <div class="right" ref="right">
        <div>
          <ul class="right__menu">
            <li class="right__list" v-for="(item, index) in destination" ref="menu" :key="index">
              <h2 class="right__list-title">{{item.country}}</h2>
              <ul ref="list">
                <li
                  v-for="(citys,index) in item.cityMenu"
                  :key="index"
                  class="right__list-info"
                  @click="selectCity(citys)"
                >
                  <div class="right__list-img">
                    <img :src="citys.pic" alt="图片" class="img">
                  </div>
                  <p class="right__list__cityname">{{citys.city}}</p>
                </li>
              </ul>
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    selectCity(citys) {
      this.keepInfo = citys
      this.$refs.desInfo.openCityInfo(citys.city);
    }
  }
}
