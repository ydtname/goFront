<template>
  <div id="app">
    <v-header :seller="this.$store.state.seller"></v-header>
    <div class="tab">
    ...
    <transition :name=" transitionName">
      <navigation>
        <router-view :seller="this.$store.state.seller" class="child-view"></router-view>
      </navigation>
    </transition>
...
export default {
  ...
  created() {
    ...
    let seller = JSON.parse(sessionStorage.getItem('seller')) || {pics:[]}
    let homeList = JSON.parse(sessionStorage.getItem('homeList'))
    this.$store.commit('showLogin', !logined)
    this.$store.commit('setSeller', seller)
    this.$store.commit('setHomeList', homeList)
    
    // this.$http.get('./api/seller').then((res) => {
    //   res = res.body;
    //   if (res.errno === ERR_OK) {
    //     this.seller = res.data || {pics:[]};
    //   };
    // })
  },
  watch: {
